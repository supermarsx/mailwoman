...
export function ComposerTools(props: ComposerToolsProps): JSX.Element {
  ...
  async function run(spec: ComposerToolSpec): Promise<void> {
    ...
    try {
      ...
      setError('Assist could not complete that just now.');
    } finally {
      setBusy(null);
    }
  }
  ...
  return (
    <Show when={visibleTools().length > 0}>
      <div class={css.field} data-module="assist-composer" aria-label="Writing help">
        <div class={css.toolbar} role="group" aria-label="Composer tools">
          <For each={visibleTools()}>
            {(spec) => (
              <div class={css.row}>
                ...
                >
                  {busy() === spec.id ? `${spec.label}…` : spec.label}
                </button>
                <Show when={spec.arg !== undefined}>
                  <label class={css.check}>
                    <span class="sr-only">{spec.arg?.label}</span>
                    <select
                      class={css.input}
                      aria-label={spec.arg?.label ?? spec.label}
        ...
        <Show when={pending()}>
          {(p) => (
            <div class={css.field}>
              <span class={css.subHeading}>Suggested edit</span>
              <div class={css.suggestion} data-testid="composer-suggestion">
                {p().text}
              </div>
              <div class={css.row}>
                <button
