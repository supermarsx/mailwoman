//! The `wasm32-wasip2` guest component (t10-e0 scaffold; e7 fills). Compiled ONLY
//! for wasm (gated by `lib.rs`).
//!
//! `message-pipeline::message-out` is the real hook (rewrite the sender to the
//! selected masked alias on send); every other export in the frozen `plugin` world
//! — including the optional PIM/parity `calendar`/`tasks`/`bridge-parity` interfaces —
//! is a trivial stub advertising no support. Scaffold: `message-out` is an identity
//! passthrough.
...
fn stub(what: &str) -> PluginError {
    PluginError::Unsupported(format!(
        "{what} not implemented in the masked-email scaffold (t10-e0)"
    ))
}

// ── The real hook: rewrite the sender to the masked alias on send (e7 fills) ────

impl pipe::Guest for Component {
    fn message_in(raw: Vec<u8>) -> Result<Vec<u8>, PluginError> {
        Ok(raw)
    }
    /// e7 rewrites the outgoing message's sender header to the selected masked
    /// alias. Scaffold: identity passthrough.
